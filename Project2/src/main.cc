...
int main(int argc, char const *argv[]) {
  ...
  // process input file
  if (election->parseInput(fname) == 0) {
  ...
  cout << "No. of seats = " << election->getNum_seats() << endl;
  cout << "No. of ballots = " << election->getNum_ballots() << endl;
  cout << "Voting method = " << ((election->getVoting_method() == 1) ?
                                "Plurality" : "Droop Quota") << endl;
  ...
  */
  ...
  cout << "Vote counting process is complete." << endl << endl;
  //cout << "Enter a name for the output file: ";
  //cin >> file_name;
  strcpy(file_name, "result");

  if (election->writeToFile(file_name) == 1) {
    cout << "Generate report file successfully" << endl;
  } else {
    cout << "Cannot generate report file" << endl;
    return 1;
  }
  ...
  */
  strcpy(file_name, "audit");
  if (election->generateAuditFile(file_name) == 1) {
    cout << "Generate audit file successfully" << endl;
  } else {
    cout << "Cannot generate audit file" << endl;
    return 1;
  }
  //election->generateAuditFile(file_name);
  //cout << "Generate the audit file successfully" << endl;

  if (election->generateReportFile("report") == 1) {
    cout << "Generate report file successfully" << endl;
  } else {
    cout << "Cannot generate report file" << endl;
    return 1;
